      ...
      <section className="relative z-10">
        <div className="flex flex-col md:flex-row md:items-baseline justify-between mb-16 backdrop-blur-sm bg-white/20 p-6 rounded-2xl border border-white/10">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm text-slate-600 font-outfit font-light leading-relaxed">
              Long Sev threads can exceed what we send to the model in one shot. The playground keeps a
              compact, structured memory (summary and key facts) in your Supabase session row, merges it on
              every request, and validates CAN-style outputs before calling the model when facts are
              incomplete. That is intentional product behavior—not generic chat memory.
            </p>
          </div>
          <p className="text-xs text-slate-400 font-mono mt-6 md:mt-0 tracking-widest uppercase shrink-0">
            CTX_RETAIN_02
        ...
        <div className="rounded-[2rem] border border-white/30 bg-white/40 backdrop-blur-xl p-8 md:p-10 shadow-xl shadow-slate-200/40">
          <div className="flex flex-col sm:flex-row sm:items-start gap-6">
            <div className="p-3 rounded-xl bg-slate-950 text-white shrink-0">
            ...
            <div className="space-y-4 text-sm text-slate-600 font-light leading-relaxed">
              <p className="text-slate-800 font-outfit font-medium text-base">
                How we keep your incident grounded
              </p>
              <ul className="list-disc pl-5 space-y-2 marker:text-slate-400">
                <li>
                  <span className="text-slate-800 font-normal">Durable session envelope</span> in Postgres:
                  messages plus a small memory object updated from each user message, reinjected into the
                  system prompt so early details survive token trimming.
                </li>
                <li>
                  <span className="text-slate-800 font-normal">No duplicate last turn</span> when the
                  client already appended the current message to history—reduces noise and token waste.
                </li>
                <li>
                  <span className="text-slate-800 font-normal">CAN grounding</span> asks for missing
                  structured fields instead of fabricating a report after context is dropped from the raw
                  transcript.
                </li>
              </ul>
              <p>
                <Link
                  ...
                  className="text-sm font-medium text-slate-950 border-b border-slate-900 pb-0.5 hover:border-emerald-500 hover:text-emerald-700 transition-colors"
                >
                  Read the full architecture and diagrams
                </Link>{' '}
                <span className="text-slate-500">or follow the narrative on</span>{' '}
                <Link
                  href="/flow"
                  className="text-sm font-medium text-slate-950 border-b border-slate-300 pb-0.5 hover:border-emerald-500 hover:text-emerald-700 transition-colors"
                >
                  Chat flow
                </Link>
                .
