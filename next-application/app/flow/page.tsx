  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-24 lg:px-12 lg:py-40">
      ...
      >
        ...
        <motion.section variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
          <div className="lg:col-span-4 sticky top-40">
            ...
            <p className="text-base text-slate-500 font-light leading-relaxed">
              Thin system prompt, bounded history, persisted memory injected before routing into model-only or
        ...
        <motion.section variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
          <div className="lg:col-span-4 sticky top-40">
            <div className="text-[10px] font-mono text-emerald-600 uppercase tracking-widest mb-4">
              Phase 01b
            </div>
            <h2 className="text-3xl font-outfit font-medium text-slate-950 mb-6 tracking-tight">
              Context retention
            </h2>
            <p className="text-base text-slate-500 font-light leading-relaxed mb-6">
              How incident threads stay coherent when transcripts are trimmed: a JSON envelope in{' '}
              <code className="text-xs font-mono text-slate-700">chat_sessions</code>, server-side memory
              injection, and CAN grounding. See also{' '}
              <Link
                href="/architecture#conversation-context"
                className="text-slate-900 underline decoration-slate-300 underline-offset-4 hover:decoration-emerald-500"
              >
                Architecture → Conversation context retention
              </Link>
              .
            </p>
          </div>
          <div className="lg:col-span-8 space-y-12">
            <MermaidDiagramCard
              ...
              description="Load envelope, merge facts, optional CAN short-circuit, trim, then generate with memory in the system prompt."
              diagram={conversationContextRetentionFlowDiagram}
              ariaLabel="Context retention flowchart"
            />
            <MermaidDiagramCard
              title="Memory sequence"
              description="Supabase read/update of envelope.memory around the model call."
              diagram={conversationMemorySequenceDiagram}
              ariaLabel="Session memory sequence diagram"
            />
          </div>
        </motion.section>
        ...
        <motion.section variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
          <div className="lg:col-span-4 lg:order-last sticky top-40">
            ...
            <p className="text-base text-slate-500 font-light leading-relaxed">
              Detailed sequence from chat submit through auth, optional envelope read, quota, trim, memory
              persistence, decision gates, optional MCP calls, and final JSON response.
            </p>
          </div>
          <div className="lg:col-span-8">
            <MermaidDiagramCard
              ...
              {[
                {
                  ...
                  desc: 'The client posts the new message, a window of conversation history, and a conversationId when the session is saved.',
                },
                {
                  title: 'Envelope + memory',
                  desc: 'With conversationId, the API loads the JSON envelope from Postgres, updates incident summary and key facts from your text, and may return early for CAN requests with incomplete facts.',
                },
                {
                  title: 'Context extraction',
                  desc: 'The route runs intent/entity extraction and reads prompt runtime settings.',
                },
                {
                  title: 'History bounding',
                  desc: 'The latest user turn is deduplicated when it already appears at the end of history; LangChain trimMessages enforces the token budget; memory is re-injected into the system string.',
                },
                {
                  title: 'Conversational vs actionable',
                  desc: 'Regex routing and action entity detection classify the request.',
                },
                {
                  title: 'MCP execution path',
                  desc: 'The route ensures MCP connectivity and exposes tools to the AI SDK.',
                },
                {
                  title: 'Response delivery',
                  desc: 'Final assistant text plus metadata are returned; the client debounces saving the full transcript under RLS.',
                },
              ].map((step, i) => (
